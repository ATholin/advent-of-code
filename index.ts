...
yargs.command('$0 [year] [day]', 'Run the day', (yargs) => {
    yargs
      .positional('year', {
	...
	import(`./src/${year}/${day}/part_one.ts`).then(part1 => {
		(async () => {
			console.log(`Result: ${chalk.green(await bench(read(Number(year), Number(day)), part1.runner))}\n`)
			import(`./src/${year}/${day}/part_two.ts`).then(part2 => {
				(async () => console.log(`Result: ${chalk.green(await bench(read(Number(year), Number(day)), part2.runner))}\n`))();
			})
		})();
	})
