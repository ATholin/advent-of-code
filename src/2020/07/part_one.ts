...
export const findBag = (toFind: string, bags: Bag[]): Bag => {
	return bags.find((bag: Bag) => bag.color === toFind) ?? {
		color: 'yeet',
		contain: []
	}
}
...
export const toBag = (bagString: string): Bag => {
	...
	const amount = s[1].replace(/[^\d,]/g, '').split(',').map((a: string) => parseInt(a, 10))

	const bc = color.map((c: string, i: number): BagContain => {
		return {
			color: c,
			amount: amount[i]
		}
...
export const runner = (input: string): number => {
	...
	return bags.map((bag: Bag) => bagContains(bag, toFind, bags)).filter((hasBag: boolean) => hasBag === true).length
};
